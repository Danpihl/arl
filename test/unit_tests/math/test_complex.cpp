...
TEST_F(ComplexTest, Multiply)
{
    ...
    const Complexd c0(0.235253, -2.252363);
    const Complexd c1(-9.345223, 4.42312);
...
TEST_F(ComplexTest, Abs)
{
    ...
    const Complexd c_unit(1.0 / std::sqrt(2.0), 1.0 / std::sqrt(2.0));
...
TEST_F(ComplexTest, ComplexToPolar)
{
    ...
    for (size_t k = 0; k < num_test_cases; k++)
    {
        ...
        const Complexd c(r * std::cos(phi), r * std::sin(phi));

        const PolarVecd p = c.toPolar();
...
TEST_F(ComplexTest, PolarToComplex)
{
    ...
    for (size_t k = 0; k < num_test_cases; k++)
    {
        ...
        const PolarVecd pv(r, phi);
        const Complexd c = pv.toComplex();
...
TEST_F(ComplexTest, PolarToComplexBackAndForth)
{
    ...
    for (size_t k = 0; k < num_test_cases; k++)
    {
        ...
        const PolarVecd pv_expected(r, phi);
        const Complexd c = pv_expected.toComplex();

        const PolarVecd pv_actual = c.toPolar();
