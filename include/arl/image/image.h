#ifndef IMAGE_CORE_H_
#define IMAGE_CORE_H_
...
template <typename T> T ImageC1<T>::min() const
{
    ...
    for (size_t idx = 1; idx < num_rows * num_cols - 1; idx++)
    {
        min_value = data[idx] < min_value ? data[idx] : min_value;
    }
...
template <typename T> T ImageC1<T>::max() const
{
    ...
    for (size_t idx = 1; idx < num_rows * num_cols - 1; idx++)
    {
        max_value = data[idx] > max_value ? data[idx] : max_value;
    }
...
void convertBetweenTypes(const ImageC1<T0>& src, ImageC1<T1>& dst)
{
    ...
    {
        dst.reallocate(src.rows(), src.cols());
    }
    ...
    for (size_t r = 0; r < src.rows(); r++)
    {
        ...
        {
            dst(r, c) = static_cast<T1>(src(r, c));
        }
    }
}
